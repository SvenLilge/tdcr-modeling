...
#ifndef PSEUDORIGIDBODYMODEL_H
#define PSEUDORIGIDBODYMODEL_H
...
// This class implements a Pseudo Rigid Body model to solve the forward kinematics of a TDCR.
// This code is written for a two segment TDCR with to three tendons per segment subject to an external load at the tip.
class PseudoRigidBodyModel
{
public:

	// Constructor to set up the Pseudo Rigid Body model with default parameters
	PseudoRigidBodyModel();
	
	// Simple destructor
	virtual ~PseudoRigidBodyModel();
	...
	//
	// Inputs:
	// output				5nx1 vector containing the moment and force equilibirium at the end of each subsegment.
	//		
	// Outputs:		
	// input				5nx1 vector containing the current values for the pseudo discrete joints of each subsegment.
	...
	// This function returns the number of pseudo discrete joints per subsegment.
	...
	//
	// Inputs:
	// inits				5nx1 vector containing the default values that are used as an initial guess for the pseudo discrete joint values for each subsegment, if continuation mode is disabled.
	...
	// It returns 5nx1 vector containing the last pseudo discrete joint values for each subsegment.
...
private:
	...
	Eigen::Matrix4d getDiskTransform(Eigen::MatrixXd &Trb, Eigen::MatrixXd var, Eigen::VectorXd length_ss, int idx_from, int idx_to);
