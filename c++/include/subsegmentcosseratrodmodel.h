...
// This class implements a Subsegment Cosserat Rod model to solve the forward kinematics of a TDCR.
// This code is written for a two segment TDCR with to three tendons per segment subject to an external load at the tip.
// Tendon paths are assumed to be only partially constrained at specific disk locations and are assumed straight between two disks.
// Each subsegment between two disks is solved as an individual Cosserat Rod subject to tendon/external loads and boundary conditions at the transition to neighbouring segments.
class SubsegmentCosseratRodModel 
{
public:

	// Constructor to set up the Subsegment Constant Curvature model with default parameters
	...
	//
	// Inputs:
	// output				6nx1 vector containing the force and moment equilibirium at the end of each subsegment.
	//		
	// Outputs:		
	// input				6nx1 vector containing the current initial values for u and v at the beginning of each subsegment.
	...
	//
	// Inputs:
	// inits				6nx1 vector containing the default values that are used as an initial guess for u and v at the beginning of each subsegment, if continuation mode is disabled.
	...
	// It returns 6nx1 vector containing the last values for u and v at the beginning of each subsegment.
